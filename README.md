...
authoritative source of version information for your project.  You can
also override the exact command used to generate the version.
...
<project>/src/<project>/version.clj, containing defs for the version
and the sha1 of the git commit object.
...
You can do this:
    ...
    (ns nifty.version)
    (def version "1.0.0")
    (def gitref "bf0c6a3bfcbe34db3dbe7e9fa3e77340b11cd99")
    $

## Configuration

By default, lein-git-version uses these git commands to determine the
version and ref (respectively):
...
Either of these commands can be overridden in your project.clj file by
using the `:git-version` lein parameter with the `:describe` and
`:ref` optional keys.  For example:

```clojure
:git-version {:describe ["git" "describe" "--long" "--match" "v*.*" "--dirty=**DIRTY**"]}
              :ref ["git" "rev-parse" "--short" "--verify" "HEAD"]}
```

These keys are entirely optional.
